...
// Get the current logged-in user's name for the "Prepared By" field
$current_user_id = (int)$_SESSION['user_id'];
$stmt_user = $pdo->prepare("SELECT name FROM users WHERE id = ?");
$stmt_user->execute([$current_user_id]);
$current_user_name = $stmt_user->fetchColumn() ?: ($_SESSION['username'] ?? 'N/A');
...
// Fetch the specific perdium transaction with all details - FIXED QUERY
$sql = "SELECT
            pt.*,
            bo.name AS owner_name,
            bo.code AS owner_code,
            bo.p_koox As project_migaq_kee_koox,
            pbo.name AS program_owner_name,  -- Added program owner name
            e.name AS emp_name,
        ...
        LEFT JOIN p_budget_owners pbo ON (pt.budget_owner_id = pbo.id AND pt.budget_type = 'program')  -- Join with program budget owners
...
try {
    ...
    $printingDateEthiopian = $ethiopianDate->format('d/m/Y');
} catch (Exception $e) {
    // Fallback to Gregorian if Ethiopian calendar library fails
    $formattedDate = date('d/m/Y');
    $printingDateEthiopian = date('d/m/Y');
}

// Determine which project name to display based on budget type
$project_migaq_kee_koox = $transaction['project_migaq_kee_koox']; // Default to governmental

// If budget type is program, use the program budget owner name
if (isset($transaction['budget_type']) && $transaction['budget_type'] === 'program') {
    $project_migaq_kee_koox = $transaction['program_owner_name'] ?? $transaction['project_migaq_kee_koox'];
}
...
<head>
    ...
    <style>
        ...
        gap: 25px; /* Creates space between each footer item */
        padding: 15px 10px; /* Adds some padding around the content */
        border-top: 1px solid #e0e0e0; /* Optional: Adds a nice separator line */
        font-family: sans-serif; /* Optional: for a clean look */
        color: #333;
    ...
    .footer-item i {
        margin-right: 8px; /* Adds a small space between the icon and the text */
        font-size: 1.1em; /* Makes the icon slightly larger than the text */
        color: #0056b3; /* A sample color for the icons */
    }
       ...
       .page-footer p {
        ...
        .government-budget-text {
  /* Increase the font size by 20% relative to the parent text */
  font-size: 1.2em; 
  
  /* You can also make it bolder if you like, as it's within a <strong> tag */
  font-weight: 700; 
}
...
<p class="header-main-title">
  <strong>
    Doolat Taamabeynih Assenti Esserih Cibta
    <span class="government-budget-text">( Government Budget )</span>
  </strong>
</p>
        ...
        <div class="value" style="width: 250px;">
            <strong><?php echo htmlspecialchars($formattedDate); ?> ዓ.ም</strong>
                ...
                <main>
                  ...
                  <div class="field-group">
    <label for="project_migaq_kee_koox">2. Taamagoli:</label>
    <div class="input-wrapper">
        <input type="text" id="project_migaq_kee_koox" name="project_migaq_kee_koox" value="<?php echo htmlspecialchars($transaction['owner_name']); ?>" style="font-weight: bold;">
                            </div>
                            </div>
<div class="field-group">
    <label for="project_migaq_kee_koox">3. Project Migaq Kee Koox:</label>
    <div class="input-wrapper">
        <input type="text" id="project_migaq_kee_koox" name="project_migaq_kee_koox" value="<?php echo htmlspecialchars($project_migaq_kee_koox); ?>" style="font-weight: bold;">
        </div>
        </div>
        <div class="field-group">
  <label for="taamabeyni_migaq_1">4. Taamabeyni Migaq:</label>
                        <div class="input-wrapper">
                            <input type="text" id="taamabeyni_migaq_1" name="taamabeyni_migaq_1" value="<?php echo htmlspecialchars($transaction['emp_name']); ?>" style="font-weight: bold;">
                            </div>
                            </div>
<div class="field-group">
    <label for="qasbikinnihgide">5. Qasbikinnihgide:</label>
    <div class="input-wrapper">
        <input type="text" id="qasbikinnihgide" name="qasbikinnihgide" value="<?php echo number_format((float)$transaction['emp_salary'], 2) . ' ብር'; ?>" style="font-weight: bold;">
    </div>
</div>
                    <div class="field-group">
                    ...
                    <div style="display:flex; justify-content: space-between; align-items: flex-end; margin-top: 15px; border-top: 1px solid #ccc; padding-top: 8px;">
                        <div>
                            ...
                            <ul style="list-style-type: none; padding-left: 8px; margin:0;">
                                <li><strong>* Lim/Maalu/Ik/Xii/Xayrektoret</strong></li>
                                <li><strong>Qaafiyat Biiro</strong></li>
                            </ul>
                        </div>
...
<p class="header-main-title">
  <strong>
    Doolat Taamabeynih Assenti Obsa Cibta
    <span class="government-budget-text">( Government Budget )</span>
  </strong>
</p>
                    </div>
...
<div class="field-group">
    <label for="Taamagoli">Taamagoli:</label>
    <div class="input-wrapper">
        <input type="text" id="Taamagoli" name="Taamagoli" value="<?php echo htmlspecialchars($transaction['owner_name']); ?>" style="font-weight: bold;">
        </div>
        </div>
    ...
    <div class="field-group">
                        ...
                        <div class="input-wrapper">
                            <input type="text" id="taamabeyni_migaq_2" name="taamabeyni_migaq_2" value="<?php echo htmlspecialchars($transaction['emp_name']); ?>" style="font-weight: bold;">
                        </div>
                    </div>
                    <div class="field-group">
                      <label for="qasbi_kinnihgide_2">Qasbi kinnihgide:</label>
                      <div class="input-wrapper">
                        <input type="text" id="qasbi_kinnihgide_2" name="qasbi_kinnihgide_2" value="<?php echo number_format((float)$transaction['emp_salary'], 2) . ' ብር'; ?>" style="font-weight: bold;">
                        </div>
                        </div>
                    ...
                    <div class="field-group">
                        ...
                        <div class="input-wrapper">
                            <input type="text" id="merraytu" name="merraytu" value="Kokkobi Merraytut" style="font-weight: bold;">
                        </div>
                    </div>
                         ...
                         <tbody>
                            <tr>
                                ...
                                <td><input type="number" value="<?php echo max(0, $total_days - 1); ?>" style="font-weight: bold;"></td>
                                 ...
                                 <td>
                                   <input type="text" value="<?php echo number_format($total_travel_allowance, 2); ?>" style="font-weight: bold;">
                                   </td>
                                   </tr>
                         </tbody>
                    </table>
                    ...
                    <div style="display: flex; justify-content: flex-end; margin-top: 8px;">
                        <div style="width: 40%;">
                            <div class="field-group">
                              ...
                              <div class="input-wrapper">
                                <span id="a_kee_b_sittat" style="font-weight: bold; border-bottom: 1.5px solid #000; display: block; padding: 1px 3px;">
                                  <?php echo number_format($table_total_last_col + $total_travel_allowance, 2); ?>
                                  </span>
                                  </div>
                                  </div>
    ...
    <!-- Column 1 -->
    <div class="field-group">
        <label>Taama Gexanum Migaqa:</label>
        <div class="input-wrapper">
            <input type="text" value="<?php echo htmlspecialchars($transaction['emp_name']); ?>">
        </div>
    </div>
    <div class="field-group">
        <label>Firma:</label>
        <div class="input-wrapper"><input type="text"></div>
            ...
            <!-- Column 2 -->
      <div class="field-group">
        <label>Cisaab Massose Migaqa:</label>
        <div class="input-wrapper">
    ...
    <div class="field-group">
        <label>Diggosse Migaqa:</label>
        <div class="input-wrapper"><input type="text" value="Abubakar Qarba Qunde"></div>
    </div>
    <div class="field-group">
        <label>Firma:</label>
        <div class="input-wrapper"><input type="text"></div>
...
<div class="printing-info" style="display: flex; justify-content: space-between; align-items: baseline;">
    
    <p style="font-size: 125%;">Printed by: <?php echo htmlspecialchars($current_user_name); ?></p>
    
 <p style="font-size: 125%;">Printing Date: <?php echo htmlspecialchars($printingDateEthiopian); ?> ዓ.ም</p>

</div>
<div class="page-footer">
    <span class="footer-item">
        <i class="fas fa-map-marker-alt"></i> Address: Semera, Ethiopia
    </span>

    <span class="footer-item">
        <i class="fas fa-phone"></i> Tel: 033-666-00-22
    </span>

    <span class="footer-item">
        <i class="fas fa-envelope"></i> Email: info@afarrhb.et
    </span>
    
   <span class="footer-item">
    <i class="fas fa-copyright"></i> Developed by: Ali Abdela
</span>
