    ...
    if ($last_new_gauge && $current_gauge < $last_new_gauge) {
        ...
        try {
            ...
            if ($budget_type === 'program') {
                // **CORRECTED LOGIC FOR PROGRAMS BUDGET**
                // Deduct from program yearly budgets (owner-level) from the new `p_budgets` table
                $stmt = $pdo->prepare("
                ...
                foreach ($rows as $r) {
                    ...
                    $use = min($avail, $left);
                    $newRem = $avail - $use;
                    $upd = $pdo->prepare("UPDATE p_budgets SET remaining_yearly = ? WHERE id = ?");
