...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Get employee salary
    $stmt = $pdo->prepare("SELECT salary, directorate FROM employees WHERE id = ?");
    ...
    try {
        ...
        // 2) Insert perdium transaction - FIXED: Added budget_code_id field
        $stmt = $pdo->prepare("
            INSERT INTO perdium_transactions (
                employee_id,
                budget_owner_id,
                budget_code_id,  -- Added this field
                city_id,
                perdium_rate,
                total_days,
                departure_date,
                ...
                total_amount,
                et_month
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)  -- Added one more parameter
        ");
        $stmt->execute([
            $employee_id, 
            $budget_owner_id, 
            $budget_code_id,  // Added this value
            $city_id,
            $perdium_rate,
            $total_days,
            $departure_date,
  ...
  <div class="sidebar bg-gradient-to-b from-blue-800 to-blue-600 text-white" id="sidebar">
    <div class="p-5">
      ...
      <ul class="space-y-2">
        ...
        <li>
          <a href="perdium.php.php" class="flex items-center p-3 text-base font-normal rounded-lg text-white bg-white/20">
              ...
              <select name="employee_id" id="employee_id" required class="w-full select2">
                ...
                <?php foreach ($employees as $e): ?>
                  <option value="<?php echo $e['id']; ?>" data-salary="<?php echo $e['salary']; ?>" data-directorate="<?php echo htmlspecialchars($e['directorate']); ?>">
                    <?php echo htmlspecialchars($e['name'] . ' - ' . $e['directorate'] . ' (' . $e['salary'] . ' Birr)'); ?>
                  </option>
                <?php endforeach; ?>
              </select>
            </div>
