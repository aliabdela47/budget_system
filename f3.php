    ...
    if ($last_new_gauge && $current_gauge < $last_new_gauge) {
        ...
        try {
            ...
            if ($budget_type === 'program') {
                ...
                // Governmental (keep your original deduction)
                // Try monthly first
                $stmt = $pdo->prepare("
                    SELECT *
                    FROM budgets
...
<body class="text-slate-700 flex bg-gray-100 min-h-screen">
  ...
  <script>
    ...
    // Reset form when switching budget source type and sync filters
    function resetFuelFormOnTypeSwitch(){
      $('#owner_id').val('').trigger('change');
      $('#et_month').val('').trigger('change');
      $('#plate_number').val('').trigger('change');
      $('#driver_name').val('');
      $('#current').val('');
      $('#journey').val('');
    ...
    function onBudgetTypeChange(){
      ...
      resetFuelFormOnTypeSwitch(); // reset and re-fetch
    }
    ...
    function onOwnerChange(){
      ...
      // Sync filter owner and refresh list
    ...
    function validateBeforeSubmit(){
      // Ensure filters sync on submit too (optional)
      syncFiltersFromForm();
      return true;
    }
    ...
    $(document).ready(function(){
      ...
      // Wire form dropdown changes to drive the list below
      $('#budget_type').on('change', function(){
        onBudgetTypeChange();
        // sync filter type and refresh list
        $('#flt_type').val($('#budget_type').val()).trigger('change.select2');
        toggleFilterMonth();
        fetchFuelList();
      });
      $('#owner_id').on('change', function(){ onOwnerChange(); });
      $('#et_month').on('change', function(){
        // sync filter month and refresh list
        $('#flt_month').val($('#et_month').val()).trigger('change.select2');
        fetchFuelList();
        loadFuelRemaining();
      });
