...
function getQuarterlyFuelBudget($pdo, $owner_id, $year, $et_month) {
    $quarter = getQuarterFromEtMonth($et_month);
    if (!$quarter) return 0;
    ...
    $months = $monthsByQuarter[$quarter];
    $placeholders = implode(',', array_fill(0, count($months), '?'));
    $params = array_merge([$owner_id, 5, $year], $months); // 5 = fuel code ID

    $stmt = $pdo->prepare("
        SELECT SUM(remaining_monthly) AS total
        FROM budgets
        WHERE owner_id = ? AND code_id = ? AND year = ? AND month IN ($placeholders)
    ");
    $stmt->execute($params);
