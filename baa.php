...
<body class="text-slate-700 flex">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select name="owner_id" id="owner_id" class="w-full select2" required onchange="updateSelectedOwner(this); refreshAvailability(); fetchBudgets();">
                ...
                <?php
                // We'll populate this dynamically via AJAX, but for initial load we need to show the correct options
  ...
  <script>
    ...
    // Function to fetch and populate owners based on budget type
    function fetchAndPopulateOwners() {
        ...
        $.ajax({
            ...
            complete: function() {
                ...
                // Trigger change to update the selected owner display
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const code  = $('#code_id');
      const month = $('#month');
      const date  = $('#adding_date');
      const type  = $('#budget_type');

      if (owner) owner.on('change', () => { refreshAvailability(); fetchBudgets(); });
      if (code)  code.on('change', () => { refreshAvailability(); fetchBudgets(); });
      if (month) month.on('change', refreshAvailability);
      if (date)  date.on('change', refreshAvailability);
      if (type)  type.on('change', () => { toggleBudgetType(); fetchAndPopulateOwners(); refreshAvailability(); fetchBudgets(); });
    });
