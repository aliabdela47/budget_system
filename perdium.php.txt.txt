...
// Get owners from both tables for the FILTER dropdown below the form
...
<body class="text-slate-700 flex bg-gray-100 min-h-screen">
  ...
  <script>
    ...
    function calculatePerdium() {
      ...
      let totalAmount = 0;
      if (days > 0 && rate > 0) {
          const A = (rate * 0.1) + (rate * 0.25) + (rate * 0.25);
          const B = A * (days - 2);
          const C = (rate * 0.1) + (rate * 0.25);
          const D = rate * (days - 1) * 0.4;
          totalAmount = A + B + C + D;
      }
    ...
    /**
     * ✅ NEW FUNCTION
     * Fetches budget owners via AJAX based on the selected budget type
     * and populates the main form's owner dropdown.
     */
    function fetchAndPopulateOwners() {
        ...
        const perdiumOwnerId = '<?php echo $perdium['budget_owner_id'] ?? ''; ?>';
        const perdiumBudgetType = '<?php echo $perdium['budget_type'] ?? ''; ?>';
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && Array.isArray(response.owners)) {
                    ...
                    if (isEdit && perdiumOwnerId && perdiumBudgetType === budgetType) {
                        ownerSelect.val(perdiumOwnerId);
                    }
                } else {
    ...
    /**
     * ✅ NEW FUNCTION
     * Shows/hides options in the FILTER owner dropdown based on the
     * selected filter budget type. This does not use AJAX.
     */
    function updateFilterOwnerOptions() {
        ...
        fltOwnerSelect.find('option').each(function() {
            ...
            // Show if it matches the type OR if it's the "Any Owner" option
            if (!optionBudgetType || optionBudgetType === budgetType) {
                option.prop('disabled', false);
            } else {
                option.prop('disabled', true);
                if (option.is(':selected')) {
                    fltOwnerSelect.val(''); // Deselect if hidden
                }
            }
        });
    ...
    function fillFormFromRow(d){
      try{
        ...
        onEmployeeChange(); // Call this to update employee details card
        ...
        calculatePerdium(); // Recalculate to ensure card is correct
    ...
    $(document).ready(function(){
      ...
      if (isEdit) {
        ...
        onEmployeeChange(); // To populate employee card and perdium rate
        loadPerdiumRemaining();
        refreshGrandTotals();
        syncFiltersFromForm();
      } else {
