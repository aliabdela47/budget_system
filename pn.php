...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validation
    if ($employee_id <= 0 || !employeeExists($pdo, $employee_id)) { set_flash('Invalid employee selected', 'error'); }
    elseif ($owner_id <= 0 || !ownerExists($pdo, $budget_type, $owner_id)) { set_flash('Invalid budget owner selected', 'error'); }
    elseif ($city_id <= 0 || !cityExists($pdo, $city_id)) { set_flash('Invalid destination city selected', 'error'); }
    elseif ($perdium_rate <= 0) { set_flash('Perdium rate must be greater than 0', 'error'); }
    ...
    elseif ($budget_type === 'governmental' && $et_month !== '' && !in_array($et_month, monthsEC(), true)) { set_flash('Invalid Ethiopian month selected', 'error'); }
    // Budget access validation
    elseif (!$is_admin && !hasBudgetAccess($pdo, $user_id, $budget_type, $owner_id)) { set_flash('You do not have access to this budget owner', 'error'); }
    else {
...
<head>
  ...
  <style>
    ...
    .swal2-timer-progress-bar {
      ...
      animation: rainbowSlide 2s linear infinite !important;
    }
...
<body class="text-slate-700 flex bg-gray-100 min-h-screen">
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      Toast.fire({
        icon: type,
        title: '<?php echo addslashes($flash_message); ?>',
        background: 'linear-gradient(135deg, var(--swal2-background), #f8fafc)'
      });
    });
    <?php endif; ?>
