...
// CORRECTED QUERY: Fetch governmental budget owners for the dropdown by joining tables
$budget_owners_sql = "SELECT DISTINCT bo.name 
                      FROM budget_owners bo
                      JOIN budgets b ON bo.id = b.owner_id
                      WHERE b.budget_type = 'governmental'
                      ORDER BY bo.name ASC";
$budget_owners_stmt = $pdo->prepare($budget_owners_sql);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['edit_id'])) { // Ensure this doesn't run for edit form
...
<head>
    ...
    <style>
        ...
        .modal-content {
            ...
            width: 90%;
            max-width: 50rem; /* Adjusted for a wider modal */
            max-height: 90vh;
            overflow-y: auto;
    ...
    <div id="editEmployeeModal" class="modal-backdrop">
        <div class="modal-content">
            <div class="p-8">
                ...
                <form id="editEmployeeForm" method="POST" enctype="multipart/form-data" class="space-y-6">
                    <input type="hidden" name="csrf" value="<?php echo htmlspecialchars($_SESSION['csrf']); ?>">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                         <div class="md:col-span-1 flex flex-col items-center justify-center">
                            ...
                            <label for="edit_photo" class="mt-4 px-4 py-2 bg-gray-200 text-gray-800 rounded-lg cursor-pointer hover:bg-gray-300 transition-colors">
                        ...
                        <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label for="edit_name" class="block text-sm font-medium text-slate-700 mb-2">Full Name (English) *</label>
                                <input type="text" name="edit_name" id="edit_name" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 modern-input transition-all">
                            </div>
                            <div>
                                <label for="edit_name_am" class="block text-sm font-medium text-slate-700 mb-2 ethio-font">ሙሉ ስም (አማርኛ)</label>
                                <input type="text" name="edit_name_am" id="edit_name_am" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 modern-input transition-all ethio-font">
                            </div>
                            <div>
                                <label for="edit_salary" class="block text-sm font-medium text-slate-700 mb-2">Salary (ETB) *</label>
                                <input type="number" step="0.01" name="edit_salary" id="edit_salary" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 modern-input transition-all">
                            </div>
                             <div>
                                <label for="edit_taamagoli" class="block text-sm font-medium text-slate-700 mb-2">Position / Title *</label>
                                <input type="text" name="edit_taamagoli" id="edit_taamagoli" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 modern-input transition-all">
                            </div>
                            <div class="md:col-span-2">
                                <label for="edit_directorate" class="block text-sm font-medium text-slate-700 mb-2">Directorate / Department *</label>
                                <select name="edit_directorate" id="edit_directorate" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 modern-input transition-all bg-white">
        ...
        $(document).ready(function() {
            ...
            function loadEmployees() {
                $.ajax({
                                ...
                                const row = `
                                    <tr data-id="${emp.id}">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-center">
                                            <button class="text-blue-600 hover:text-blue-900 transition-colors duration-200 edit-btn" title="Edit"><i class="fas fa-edit"></i></button>
                                            <button class="text-red-600 hover:text-red-900 transition-colors duration-200 ml-2 delete-btn" title="Delete"><i class="fas fa-trash-alt"></i></button>
                                            <button class="text-green-600 hover:text-green-900 transition-colors duration-200 ml-2 print-btn" title="Print"><i class="fas fa-print"></i></button>
                                        </td>
                                    </tr>
                                `;
                                tbody.append(row);
                    ...
                    error: function() {
                        $('#employeeTableBody').html('<tr><td colspan="6" class="p-8 text-center text-red-500">Failed to load employee data.</td></tr>');
                    }
                });
            }
            ...
            modal.on('click', function(e) { if ($(e.target).is(modal)) hideModal(); });

            function openEditModalFor(empId) {
                 $.ajax({
                    url: 'ajax_get_employee_details.php',
                    type: 'GET',
                    data: { id: empId },
                    dataType: 'json',
                    success: function(response) {
                        if (response.success) {
                            ...
                            Swal.fire('Error', response.error || 'Could not fetch employee details.', 'error');
                        }
                    },
                     error: function() {
            ...
            $('#employeeTableBody').on('click', 'tr', function(e) {
                ...
                // Do not open modal if a button inside the row was the click target
                if ($(e.target).is('button, i') || $(e.target).closest('button').length) {
                    return;
                }
                if (empId) {
                    openEditModalFor(empId);
                }
            });

            $('#employeeTableBody').on('click', '.edit-btn', function(e) {
                e.stopPropagation();
                const empId = $(this).closest('tr').data('id');
                openEditModalFor(empId);
            });

            $('#employeeTableBody').on('click', '.delete-btn', function(e) {
                e.stopPropagation();
                const empId = $(this).closest('tr').data('id');
                Swal.fire({
                    title: 'Are you sure?',
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    ...
                                    Swal.fire('Error!', response.error || 'Could not delete employee.', 'error');
                                }
                            },
                             error: function() {
            ...
            $('#employeeTableBody').on('click', '.print-btn', function(e) {
                e.stopPropagation();
                const empId = $(this).closest('tr').data('id');
                 $.ajax({
                    url: 'ajax_get_employee_details.php',
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            printWindow.document.write(`
                                <html>
                                <head>
                                    <title>Print Employee: ${emp.name}</title>
                                    <script src="https://cdn.tailwindcss.com"><\/script>
                                    <style> .ethio-font { font-family: 'Nyala', 'Abyssinica SIL', 'GF Zemen', sans-serif; } body { -webkit-print-color-adjust: exact; } </style>
                                </head>
                                <body class="bg-gray-100 p-8">
                                    <div class="max-w-2xl mx-auto bg-white p-10 rounded-lg shadow-xl">
                                        <div class="text-center mb-8">
                                            <img src="${emp.photo || 'https://placehold.co/128x128/EFEFEF/AAAAAA&text=No+Photo'}" class="w-32 h-32 rounded-full mx-auto object-cover border-4 border-gray-200 shadow-md">
                                            <h1 class="text-3xl font-bold text-gray-800 mt-4">${emp.name}</h1>
                                            <p class="text-xl text-gray-600">${emp.taamagoli}</p>
                                        </div>
                                        <div class="border-t border-gray-200 pt-6">
                                            <dl class="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
                                                <div class="sm:col-span-1"><dt class="text-sm font-medium text-gray-500">Amharic Name</dt><dd class="mt-1 text-lg text-gray-900 ethio-font">${emp.name_am || 'N/A'}</dd></div>
                                                <div class="sm:col-span-1"><dt class="text-sm font-medium text-gray-500">Department</dt><dd class="mt-1 text-lg text-gray-900">${emp.directorate}</dd></div>
                                                <div class="sm:col-span-2"><dt class="text-sm font-medium text-gray-500">Salary</dt><dd class="mt-1 text-2xl font-semibold text-gray-900">${salaryFormatted}</dd></div>
                                            </dl>
                                        </div>
                                    </div>
                            ...
                            Swal.fire('Error', 'Could not fetch data for printing.', 'error');
                        }
                    },
                    error: function() {
                         Swal.fire('Error!', 'An unexpected error occurred while fetching details for printing.', 'error');
                    }
                });
            });
            ...
            // Handle Edit Form Submission
             $('#editEmployeeForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            hideModal();
                            Swal.fire({ icon: 'success', title: 'Updated!', text: 'Employee details saved successfully.', timer: 2000, showConfirmButton: false });
                            loadEmployees();
                        } else {
                            Swal.fire('Error!', response.error || 'Could not update employee.', 'error');
                        }
                    },
                    error: function() {
                         Swal.fire('Error!', 'An unexpected error occurred.', 'error');
             ...
             $('#edit_photo').on('change', function(event) {
                const [file] = event.target.files;
                if (file) {
                    $('#edit_photoPreview').attr('src', URL.createObjectURL(file));
                }
            });
