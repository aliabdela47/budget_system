...
// Set page title
$pageTitle = "Dashboard - AFAR-RHB Financial System";
...
$allocRows = [];
$sqlAlloc = "SELECT owner_id, SUM(yearly_amount) AS alloc FROM budgets $where_clause GROUP BY owner_id";
...
usort($directorate_data, function($a, $b) {
...
$params_t = []; $where_t = '';
if ($filter_owner > 0) {
    $where_t .= " AND owner_id = ?"; $params_t[] = $filter_owner;
}
if ($filter_code > 0) {
    $where_t .= " AND code_id = ?"; $params_t[] = $filter_code;
}
if ($start_dt) {
    $where_t .= " AND date >= ?"; $params_t[] = $start_dt;
}
if ($end_dt) {
    $where_t .= " AND date <= ?"; $params_t[] = $end_dt;
}
...
if ($includeP) {
    $params_p = []; $where_p = '';
    if ($filter_owner > 0) {
        $where_p .= " AND budget_owner_id = ?"; $params_p[] = $filter_owner;
    }
    if ($start_dt) {
        $where_p .= " AND created_at >= ?"; $params_p[] = $start_dt;
    }
    if ($end_dt) {
        $where_p .= " AND created_at <= ?"; $params_p[] = $end_dt;
    }
...
if ($includeF) {
    $params_f = []; $where_f = '';
    if ($filter_owner > 0) {
        $where_f .= " AND owner_id = ?"; $params_f[] = $filter_owner;
    }
    if ($start_dt) {
        $where_f .= " AND date >= ?"; $params_f[] = $start_dt;
    }
    if ($end_dt) {
        $where_f .= " AND date <= ?"; $params_f[] = $end_dt;
    }
...
$where_trans = '';
if ($filter_owner > 0) {
    $where_trans .= " AND t.owner_id = ?"; $params_recent[] = $filter_owner;
}
if ($filter_code > 0) {
    $where_trans .= " AND t.code_id = ?"; $params_recent[] = $filter_code;
}
if ($start_dt) {
    $where_trans .= " AND t.date >= ?"; $params_recent[] = $start_dt;
}
if ($end_dt) {
    $where_trans .= " AND t.date <= ?"; $params_recent[] = $end_dt;
}

$where_perdiem = '';
if ($includeP) {
    if ($filter_owner > 0) {
        $where_perdiem .= " AND p.budget_owner_id = ?"; $params_recent[] = $filter_owner;
    }
    if ($start_dt) {
        $where_perdiem .= " AND p.created_at >= ?"; $params_recent[] = $start_dt;
    }
    if ($end_dt) {
        $where_perdiem .= " AND p.created_at <= ?"; $params_recent[] = $end_dt;
    }
} else {
...
if ($includeF) {
    if ($filter_owner > 0) {
        $where_fuel .= " AND f.owner_id = ?"; $params_recent[] = $filter_owner;
    }
    if ($start_dt) {
        $where_fuel .= " AND f.date >= ?"; $params_recent[] = $start_dt;
    }
    if ($end_dt) {
        $where_fuel .= " AND f.date <= ?"; $params_recent[] = $end_dt;
    }
} else {
...
<body class="text-slate-700 dark:text-slate-200">
    <?php include 'includes/sidebar-component.php'; ?>
    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="p-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                    <div>
                        <h1 class="text-2xl md:text-3xl font-extrabold">
                            Welcome, <span class="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary"><?php echo h($user_name); ?></span>
                        </h1>
                        <p class="mt-2 text-slate-600 dark:text-slate-400">
                            Here's the latest financial overview of your bureau.
                        </p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <div class="text-2xl font-extrabold mt-1">
                                <?php echo fmt($total_allocated); ?> ETB
                            </div>
                        </div>
                        <div class="p-3 rounded-xl bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300">
                ...
                <div class="glass card kpi p-6 relative bg-gradient-to-br from-emerald-50 to-white dark:from-emerald-900/20 dark:to-slate-900">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <div class="text-2xl font-extrabold mt-1">
                                <?php echo fmt($total_used); ?> ETB
                            </div>
                            <div class="progress mt-2">
                                <div style="width: <?php echo round($utilization_percentage, 1); ?>%; background: linear-gradient(90deg, #10b981, #059669)"></div>
                            </div>
                        </div>
                        <div class="p-3 rounded-xl bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-300">
                ...
                <div class="glass card kpi p-6 relative bg-gradient-to-br from-sky-50 to-white dark:from-sky-900/20 dark:to-slate-900">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <div class="text-2xl font-extrabold mt-1">
                                <?php echo fmt($total_remaining); ?> ETB
                            </div>
                            <div class="progress mt-2">
                                <div style="width: <?php echo round($remaining_percentage, 1); ?>%; background: linear-gradient(90deg, #3b82f6, #06b6d4)"></div>
                            </div>
                        </div>
                        <div class="p-3 rounded-xl bg-sky-100 dark:bg-sky-900 text-sky-700 dark:text-sky-300">
                ...
                <div class="glass card kpi p-6 relative bg-gradient-to-br from-violet-50 to-white dark:from-violet-900/20 dark:to-slate-900">
                    <div class="flex justify-between items-start">
                        <div>
                            <div class="text-slate-500 dark:text-slate-400">
                                Critical Monthly Alerts
                            </div>
                            <div class="text-2xl font-extrabold mt-1">
                                <?php echo (int)$critically_low_count; ?>
                            </div>
                            <div class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                Remaining ≤ 10% of monthly
                            </div>
                        </div>
                        <div class="p-3 rounded-xl bg-violet-100 dark:bg-violet-900 text-violet-700 dark:text-violet-300">
                ...
                <div class="glass card p-6">
                    ...
                    <ul class="space-y-3 text-sm">
                        ...
                        <li class="flex items-start gap-2">
                            ...
                            <div>
                                <b><?php echo (int)$critically_low_count; ?></b> monthly budgets are critically low (≤10% remaining).
                            </div>
                        </li>
                        <li class="flex items-start gap-2">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="p-3 rounded-lg border border-slate-200 dark:border-slate-700">
                            ...
                            <div class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                <?php echo fmt($a); ?> allocated • <?php echo $pct; ?>% used
                            </div>
                            <div class="progress mt-2">
                                <div style="width: <?php echo $pct; ?>%; background:linear-gradient(90deg,#6366f1,#8b5cf6)"></div>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    </div>
            ...
            <footer class="mt-10 text-center text-slate-500 dark:text-slate-400 text-sm">
                <p>
                    &copy; <span id="currentYear"></span> Budget System • Built with care for the Afar Health Bureau
                </p>
            </footer>
        </div>
    </div>
