...
$cities = $pdo->query("SELECT * FROM cities ORDER BY name_amharic")->fetchAll(PDO::FETCH_ASSOC);


// Get perdium rate (assuming it's stored in a settings table or constant)
$perdium_rate = 1000; // Default value, replace with your actual logic
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $total_days = (int)$interval->format('%a') + 1; // Including both departure and arrival days
...
<body class="text-slate-700 flex bg-gray-100 min-h-screen">
    ...
    <div class="p-6">
      ...
      <div class="bg-white rounded-xl p-6 shadow-sm mb-8">
              ...
              <select name="owner_id" id="owner_id" required class="w-full select2">
                ...
                <?php foreach ($owners as $o): ?>
                  <option value="<?php echo $o['id']; ?>"
                          ...
                          <?php
                          $sel = false;
                          if (isset($perdium) && $perdium['budget_owner_id'] == $o['id']) $sel = true;
        ...
        <?php endforeach; ?>
            ...
            echo $sel ? 'selected' : '';
            ?>>
            <?php echo htmlspecialchars($c['name_english']); ?>
        </option>
    <?php endforeach; ?>
</select>
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              ?>" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" onchange="calculatePerdium()">
            </div>
            <div>
  ...
  <script>
    ...
    function calculatePerdium() {
      const departure = new Date($('#departure_date').val());
    ...
    function onEmployeeChange() {
      ...
      if (parts.length > 1) {
        ...
        $('#employee_department_display').text('Department: ' + (parts[2] || '-'));
      } else {
        $('#employee_position_display').text('-');
        $('#employee_department_display').text('Department: -');
      }
