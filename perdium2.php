...
// Helpers
function ecYear(): int { return (int)date('Y') - 8; } // consider replacing with a proper EC lib
...
// Best-effort reversal for legacy transactions with no ledger
function reverseLegacy(PDO $pdo, array $tx): void {
    ...
    if ($tx['budget_type'] === 'program') {
        ...
        // Fallback legacy: budgets table (program rows)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($budget_type === 'governmental' && $et_month !== '' && !in_array($et_month, monthsEC(), true)) { set_flash('Invalid Ethiopian month', 'error'); }
        ...
        // Policy: disallow creating any new per diem while the employee is currently active (even for future travel)
        ...
        // Overlap check for selected dates
...
<body class="text-slate-700 flex bg-gray-100 min-h-screen">
  ...
  <script>
    ...
    function resetPerdiumFormOnTypeSwitch(){
      ...
      activeBlock = false; overlapConflict = false;
    ...
    function validateBeforeSubmit(){
      if (activeBlock) { alert('Employee is currently on a per diem. Cannot create a new one yet.'); return false; }
      if (overlapConflict) { alert('Selected dates overlap with another per diem for this employee.'); return false; }
      syncFiltersFromForm();
      return true;
    }
      ...
      $.get('ajax_check_employee_perdium.php', { employee_id: empId, mode: 'active' }, function(resp){
        try{
          ...
          if (j.active) {
            ...
            activeBlock = false;
            $('#employeeActiveWarning').addClass('hidden');
            $('#submitBtn').prop('disabled', overlapConflict);
          }
        }catch(e){
          activeBlock = false;
          $('#employeeActiveWarning').addClass('hidden');
    ...
    function checkOverlapForDates(){
      ...
      if (!empId || !dep || !arr) {
        ...
        $('#employeeOverlapWarning').addClass('hidden');
        $('#submitBtn').prop('disabled', activeBlock);
        return;
      }
