...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        ...
        try {
            ...
            $pdo->commit();
            set_flash('Perdium transaction deleted', 'success');
        } catch (Exception $e) {
            $pdo->rollBack();
    ...
    elseif ($perdium_rate <= 0) { set_flash('Perdium rate must be > 0', 'error'); }
    ...
    elseif ($budget_type === 'governmental' && $et_month !== '' && !in_array($et_month, monthsEC(), true)) { set_flash('Invalid Ethiopian month', 'error'); }
        ...
        if (isEmployeeActive($pdo, $employee_id, $is_update ? $id : null)) {
            set_flash('This employee is currently on a perdium; you can create a new one only after the current end date.', 'error');
        } elseif (hasOverlap($pdo, $employee_id, $departure_date, $arrival_date, $is_update ? $id : null)) {
            set_flash('Selected dates overlap with another perdium for this employee.', 'error');
        } else {
            try {
                $pdo->beginTransaction();

                $year = ecYear();
                $code_id = 6; // Perdium budget code
                ...
                if ($budget_type === 'program') {
                    ...
                    set_flash($is_update ? 'Program perdium transaction updated' : 'Program perdium transaction added', 'success');
                    ...
                    insertAllocations($pdo, $perdium_id, $allocs);
                    set_flash($is_update ? 'Perdium transaction updated' : 'Perdium transaction added', 'success');
                }
  ...
  <script>
    ...
    function updateDates() {
      ...
      if (totalDays > 0) {
        ...
        if ($('#departure_date').val()) {
          ...
          arrivalDate.setDate(departureDate.getDate() + Math.max(0, totalDays - 1)); // inclusive
    ...
    function fetchAndPopulateOwners(preselectId=null) {
      ...
      $.ajax({
        ...
        error: function() { ownerSelect.html('<option value="">Error loading owners</option>'); },
        complete: function() {
    ...
    function setBudgetTypeUI(type) {
      ...
      applyOfficerLock(); // maintain lock across type changes
    }
    ...
    function onEmployeeChange() {
      if (filling) return;
      // Unlock on employee change; will lock again if conflicts found
    ...
    function validateBeforeSubmit(){
      if (isOfficer && (lockForOfficer || activeBlock || overlapConflict)) {
        alert('This employee has an active or overlapping perdium. Fields are locked and submission is blocked.');
        return false;
      }
      syncFiltersFromForm();
      return true;
    ...
    // Support your page toggle button; sidebar.js also handles desktop/mobile
