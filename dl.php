...
// Ethiopian date conversion function (fixed version)
function gregorianToEthiopian($date) {
    ...
    // Calculate day of the year in Gregorian calendar
    $startOfYear = new DateTime("$year-01-01");
    $diff = $gregorian->diff($startOfYear);
    $dayOfYear = (int)$diff->days + 1;
    
    // Calculate Ethiopian month and day
    if ($dayOfYear <= 105) {
        $ethiopianMonth = 9; // September (Meskerem)
        $ethiopianDay = $dayOfYear + 5;
    } else {
        $remainingDays = $dayOfYear - 106;
        $ethiopianMonth = (int)($remainingDays / 30) + 10;
        $ethiopianDay = ($remainingDays % 30) + 1;
    ...
    <div class="main-content" id="mainContent">
        <div class="p-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 p-6 bg-white rounded-xl shadow-sm">
                <div>
                    ...
                    <div class="flex items-center mt-3 bg-indigo-100 rounded-lg p-2 max-w-md">
                        ...
                        <span class="text-indigo-800 ethiopic font-semibold">
                ...
                <div class="bg-white rounded-xl p-6 card-hover">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold text-slate-800 mt-1"><?php echo number_format($remaining_budget, 2); ?> ETB</h3>
                        </div>
            ...
            <footer class="mt-10 text-center text-slate-500 text-sm">
                <p>Developed/Powered by: <a href="https://ali.et" class="text-indigo-600 hover:text-indigo-700 font-medium" target="_blank">Ali Abdela</a> - All rights reserved <?php echo date('Y'); ?></p>
            </footer>
        </div>
    </div>
