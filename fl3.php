...
<head>
  ...
  <style>
    ...
    body { font-family: 'Inter', sans-serif; background: linear-gradient(135deg,#f1f5f9 0%,#e2e8f0 100%); color:#334155; }
    ...
    .sidebar { width:260px; position:fixed; left:0; top:0; height:100vh; background:linear-gradient(180deg,#4f46e5 0%,#7c3aed 100%); color:#fff; z-index:1000; transition:transform .3s; }
    .sidebar.collapsed { transform:translateX(-260px); }
    .main-content { margin-left:260px; width:calc(100% - 260px); transition:margin-left .3s; }
    .main-content.expanded { margin-left:0; width:100%; }
    .input-group { display:flex; align-items:center; padding:.5rem .75rem; border:1px solid #d1d5db; border-radius:.5rem; background:transparent; transition:all .3s; }
    .input-group:focus-within { border-color:#4f46e5; box-shadow:0 0 0 1px #4f46e5; }
    .btn-primary { background-color:#4f46e5; color:#fff; padding:.75rem 1rem; border-radius:.5rem; font-weight:500; border:none; cursor:pointer; transition:background-color .2s; }
    .btn-primary:hover { background-color:#4338ca; }
    .btn-secondary { background-color:#6b7280; color:#fff; padding:.5rem 1rem; border-radius:.5rem; font-weight:500; border:none; cursor:pointer; transition:background-color .2s; }
    .btn-secondary:hover { background-color:#4b5563; }
    .btn-danger { background-color:#ef4444; color:#fff; padding:.5rem 1rem; border-radius:.5rem; font-weight:500; border:none; cursor:pointer; transition:background-color .2s; }
    .btn-danger:hover { background-color:#dc2626; }
    .btn-info { background-color:#06b6d4; color:#fff; padding:.5rem 1rem; border-radius:.5rem; font-weight:500; border:none; cursor:pointer; transition:background-color .2s; }
    .btn-info:hover { background-color:#0891b2; }
    .select2-container--default .select2-selection--single { height:42px; border:1px solid #d1d5db; border-radius:.5rem; padding:.5rem .75rem; background:transparent; }
    .select2-container--default .select2-selection--single:focus { border-color:#4f46e5; box-shadow:0 0 0 1px #4f46e5; }
    .select2-container--default .select2-selection__rendered { line-height:42px; padding-left:0; }
    .select2-container--default .select2-selection__arrow { height:40px; }
  </style>
</head>
  ...
  <div class="sidebar" id="sidebar">
    <div class="p-5">
      <div class="flex items-center justify-center mb-8">
        ...
        <h2 class="text-xl font-bold">Budget System</h2>
      </div>
      <ul class="space-y-2">
        <li><a href="dashboard.php" class="flex items-center p-3 text-white/80 hover:bg-white/10 rounded-lg"><i class="fas fa-tachometer-alt w-5"></i><span class="ml-3">Dashboard</span></a></li>
        <li><a href="budget_adding.php" class="flex items-center p-3 text-white/80 hover:bg-white/10 rounded-lg"><i class="fas fa-plus-circle w-5"></i><span class="ml-3">Budget Adding</span></a></li>
        <li><a href="settings_owners.php" class="flex items-center p-3 text-white/80 hover:bg-white/10 rounded-lg"><i class="fas fa-cog w-5"></i><span class="ml-3">Settings</span></a></li>
        <li><a href="transaction.php" class="flex items-center p-3 text-white/80 hover:bg-white/10 rounded-lg"><i class="fas fa-exchange-alt w-5"></i><span class="ml-3">Transaction</span></a></li>
        <li><a href="fuel_management.php" class="flex items-center p-3 text-white bg-white/20 rounded-lg"><i class="fas fa-gas-pump w-5"></i><span class="ml-3">Fuel Management</span></a></li>
        <li><a href="users_management.php" class="flex items-center p-3 text-white/80 hover:bg-white/10 rounded-lg"><i class="fas fa-users w-5"></i><span class="ml-3">Users Management</span></a></li>
        <li><a href="logout.php" class="flex items-center p-3 text-white/80 hover:bg-white/10 rounded-lg"><i class="fas fa-sign-out-alt w-5"></i><span class="ml-3">Logout</span></a></li>
      </ul>
    </div>
  </div>
        ...
        <form method="post" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select name="owner_id" class="input-group" required onchange="loadFuelRemaining()">
            ...
            <div>
              ...
              <select name="et_month" id="month" class="input-group ethiopic" required onchange="updateQuarter(); loadFuelRemaining();">
                <option value="">Select month…</option>
                <?php foreach ($etMonths as $m): ?>
                  <option value="<?php echo $m; ?>" <?php echo ($fuel && $fuel['et_month']==$m)?'selected':''; ?>>
                    <?php echo $m; ?>
                  </option>
                <?php endforeach; ?>
              </select>
              <div class="mt-2">
                <label class="block text-sm font-medium text-slate-700">Quarter: <span id="quarter_label">0</span></label>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select name="plate_number" id="plate_number" class="input-group select2" required onchange="fetchLastGauge(this.value)">
            ...
            <div>
              ...
              <input type="text" name="driver_name" class="input-group" value="<?php echo htmlspecialchars($fuel['driver_name'] ?? ''); ?>" required>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <input type="number" step="0.01" id="current" name="current_gauge" class="input-group" value="<?php echo htmlspecialchars($fuel['current_gauge'] ?? ''); ?>" onchange="calculateFuel()" required>
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Journey Distance (Km)</label>
              <input type="number" step="0.01" id="journey" name="journey_distance" class="input-group" value="<?php echo htmlspecialchars($fuel['journey_distance'] ?? ''); ?>" onchange="calculateFuel()" required>
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Fuel Price /L</label>
              <input type="number" step="0.01" id="price" name="fuel_price" class="input-group" value="<?php echo htmlspecialchars($fuel['fuel_price'] ?? $last_price); ?>" onchange="calculateFuel()" required>
            </div>
          </div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
  ...
  <div class="bg-white rounded-xl shadow-sm p-4 border border-slate-200">
    <h3 class="text-sm font-semibold text-slate-600 mb-1">Remaining Monthly Fuel Budget</h3>
    <p class="text-2xl font-bold text-primary"><?php echo number_format($budget['remaining_monthly'] ?? 0, 2); ?> ETB</p>
  </div>

  <!-- Quarterly Budget Card -->
  <div class="bg-white rounded-xl shadow-sm p-4 border border-slate-200">
    <h3 class="text-sm font-semibold text-slate-600 mb-1">Remaining Quarterly Fuel Budget</h3>
    <p class="text-2xl font-bold text-primary"><?php echo number_format($budget_quarterly['remaining_quarterly'] ?? 0, 2); ?> ETB</p>
  </div>

  <!-- Yearly Budget Card -->
  <div class="bg-white rounded-xl shadow-sm p-4 border border-slate-200">
    <h3 class="text-sm font-semibold text-slate-600 mb-1">Remaining Yearly Fuel Budget</h3>
    <p class="text-2xl font-bold text-primary"><?php echo number_format($budget_yearly['remaining_yearly'] ?? 0, 2); ?> ETB</p>
  </div>
</div>
          ...
          <div class="flex space-x-4 mt-6">
            <button type="submit" class="btn-primary"><?php echo $fuel ? 'Update' : 'Save'; ?></button>
      ...
      <div class="bg-white rounded-xl p-6 card-hover">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-xl font-bold">Existing Fuel Transactions</h2>
        </div>
                ...
                <tr class="border-b border-slate-200 hover:bg-slate-50">
                  ...
                  <td class="px-4 py-2">
                    <div class="flex space-x-2">
                      <a href="?action=edit&id=<?php echo $f['id']; ?>" class="btn-secondary btn-sm"><i class="fas fa-edit mr-1"></i>Edit</a>
                      <a href="?action=delete&id=<?php echo $f['id']; ?>" class="btn-danger btn-sm" onclick="return confirm('Are you sure?')"><i class="fas fa-trash mr-1"></i>Delete</a>
                    </div>
                  </td>
                </tr>
              <?php endforeach; ?>
  ...
  <script>
    ...
    function loadFuelRemaining() {
      const ownerId = document.querySelector('[name="owner_id"]').value;
      const month   = document.querySelector('[name="et_month"]').value;
      const codeId  = 5;
