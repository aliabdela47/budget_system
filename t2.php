...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($budget_yearly || $budget) {
        ...
        $remaining_monthly = $budget ? $budget['remaining_monthly'] : 0;
        $remaining_yearly = $budget_yearly ? $budget_yearly['remaining_yearly'] : 0;
        ...
        $calculated_remaining_year = $remaining_yearly - $trans_year;
        ...
        $calculated_remaining_month = $remaining_monthly - $trans_month;

        // Calculate quarterly amount by summing remainingmonthly for the quarter
        ...
        $quarterly_remaining = $stmt->fetchColumn() ?: 0;
        ...
        $calculated_remaining_quarter = $quarterly_remaining - $trans_quarter;

        if (!$budget) {
            $calculated_remaining_month = $calculated_remaining_year;
        }

        if ($quarterly_remaining == 0) {
        ...
        if ($amount < 0) {
            ...
            try {
                // Update the remainingmonthly column instead of monthly_amount
                if ($amount > 0 && $budget) {
                    $new_remaining_monthly = $remaining_monthly - $amount;
                    $stmt = $pdo->prepare("UPDATE budgets SET remaining_monthly = ? WHERE id = ?");
                    $stmt->execute([$new_remaining_monthly, $budget['id']]);
                }
                
                // Update yearly remaining if no monthly budget exists
                if ($amount > 0 && $budget_yearly && !$budget) {
                    $new_remaining_yearly = $remaining_yearly - $amount;
                ...
                if (isset($_POST['id']) && $_POST['action'] == 'update') {
                    $stmt = $pdo->prepare("UPDATE transactions SET owner_id = ?, code_id = ?, employee_name = ?, ordered_by = ?, reason = ?, amount = ?, et_month = ?, quarter = ?, remaining_monthly = ?, remaining_quarterly = ?, remaining_yearly = ? WHERE id = ?");
                    ...
                    $stmt = $pdo->prepare("INSERT INTO transactions (owner_id, code_id, employee_name, ordered_by, reason, created_by, amount, et_month, quarter, remaining_monthly, remaining_quarterly, remaining_yearly, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
                        ...
                        <div>
                            ...
                            <select name="code_id" class="input-group" onchange="loadAvailable()" required>
                                ...
                                <?php foreach ($codes as $c): ?>
                                    <option value="<?php echo $c['id']; ?>" <?php echo ($trans && $trans['code_id'] == $c['id']) ? 'selected' : ''; ?>>
        ...
        function loadAvailable() {
            ...
            if (ownerId && codeId && month) {
                fetch(`get_remaining.php?owner_id=${ownerId}&code_id=${codeId}&month=${encodeURIComponent(month)}`)
                    ...
                    .then(data => {
                        document.getElementById('remaining_monthly').textContent = data.remaining_month.toFixed(2);
                        document.getElementById('remaining_quarterly').textContent = data.remaining_quarter.toFixed(2);
                        document.getElementById('remaining_yearly').textContent = data.remaining_year.toFixed(2);
                    })
