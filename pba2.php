...
// Get the selected budget type for filtering (default to governmental)
$selected_budget_type = isset($_GET['budget_type']) ? $_GET['budget_type'] : 'governmental';

// Fetch budgets based on selected owner, code, and budget type
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
                ...
                if ($yearly_amount > 0) {
                    ...
                    if (!$yearly_budget) {
                        ...
                        if ($monthly_amount > $yearly_budget['remaining_yearly']) {
                            ...
                            $pdo->exec("LOCK TABLES budgets WRITE");
                        } else {
        ...
        header("Location: budget_adding.php?budget_type=" . $budget_type . "&owner_id=" . $owner_id . "&code_id=" . $code_id);
        exit;
    } catch (Exception $e) {
        $pdo->rollBack();
...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    $stmt->execute([$_GET['id']]);
    $budget_to_delete = $stmt->fetch();

    $pdo->beginTransaction();
    try {
        if ($budget_to_delete['budget_type'] == 'governmental' && $budget_to_delete['monthly_amount'] > 0) {
            ...
            $stmt->execute([$budget_to_delete['monthly_amount'], $budget_to_delete['owner_id'], $budget_to_delete['code_id'], $budget_to_delete['year']]);
        }
        $stmt = $pdo->prepare("DELETE FROM budgets WHERE id = ?");
        $stmt->execute([$_GET['id']]);
        $pdo->commit();
        $message = 'Budget deleted';
        header('Location: budget_adding.php?budget_type=' . $budget_to_delete['budget_type'] . '&owner_id=' . $budget_to_delete['owner_id'] . '&code_id=' . $budget_to_delete['code_id']);
        exit;
    } catch (Exception $e) {
        $pdo->rollBack();
...
<body class="text-slate-700 flex">
        ...
        <div class="p-6">
            ...
            <div class="bg-white rounded-xl p-6 card-hover mb-8">
                <div class="budget-type-tabs">
                    <div class="budget-type-tab <?php echo $selected_budget_type == 'governmental' ? 'active' : ''; ?>" onclick="changeBudgetType('governmental')">
                        Governmental Budgets
                    </div>
                    <div class="budget-type-tab <?php echo $selected_budget_type == 'program' ? 'active' : ''; ?>" onclick="changeBudgetType('program')">
                        Program Budgets
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl p-6 card-hover">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-xl font-bold text-slate-800">
                        Existing <?php echo $selected_budget_type == 'governmental' ? 'Governmental' : 'Program'; ?> Budgets
                    </h2>
                </div>
                                    ...
                                    <tr class="border-b border-slate-200 hover:bg-slate-50">
                                        ...
                                        <td class="px-4 py-2">
                                            <div class="flex space-x-2">
                                                <a href="?action=edit&id=<?php echo $b['id']; ?>&budget_type=<?php echo $selected_budget_type; ?>" class="btn-secondary btn-sm">
                                                    <i class="fas fa-edit mr-1"></i> Edit
                                                </a>
                                                <a href="?action=delete&id=<?php echo $b['id']; ?>&budget_type=<?php echo $selected_budget_type; ?>" class="btn-danger btn-sm" onclick="return confirm('Are you sure?')">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="9" class="px-4 py-4 text-center text-slate-500">
                                        No <?php echo $selected_budget_type == 'governmental' ? 'governmental' : 'program'; ?> budgets found.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
    ...
    <script>
        ...
        // Change budget type for filtering
        function changeBudgetType(type) {
            window.location.href = `budget_adding.php?budget_type=${type}`;
        }
