...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($last_new_gauge && $current_gauge < $last_new_gauge) {
        ...
        try {
            // Find the budget code ID for "Sansii kee Sukutih 6217"
            $stmt = $pdo->prepare("SELECT id FROM budget_codes WHERE code = ? OR name LIKE ? LIMIT 1");
            $stmt->execute(['Sansii kee Sukutih 6217', '%Sansii kee Sukutih 6217%']);
            $fuel_code_id = $stmt->fetchColumn();

            if ($fuel_code_id) {
                // Fetch the relevant budget (assuming yearly budget for simplicity)
                $stmt = $pdo->prepare("SELECT * FROM budgets WHERE owner_id = ? AND code_id = ? AND year = ? AND monthly_amount = 0");
                $stmt->execute([$owner_id, $fuel_code_id, date('Y') - 8]);
                $budget = $stmt->fetch();

                if ($budget) {
                    $new_remaining_yearly = $budget['remaining_yearly'] - $total_amount;
                    if ($new_remaining_yearly < 0) {
                        throw new Exception('Insufficient remaining yearly budget for fuel transaction.');
                    }

                    // Update the budget remaining amount
                    $stmt = $pdo->prepare("UPDATE budgets SET remaining_yearly = ? WHERE id = ?");
                    $stmt->execute([$new_remaining_yearly, $budget['id']]);
                } else {
                    // If no yearly budget, check for monthly budget
                    ...
                    $stmt->execute([$owner_id, $fuel_code_id, date('Y') - 8, $et_info['etMonth']]);
                    $budget = $stmt->fetch();

                    if ($budget) {
                        $new_remaining_monthly = $budget['remaining_monthly'] - $total_amount;
                        if ($new_remaining_monthly < 0) {
                            throw new Exception('Insufficient remaining monthly budget for fuel transaction.');
                        }

                        $stmt = $pdo->prepare("UPDATE budgets SET remaining_monthly = ? WHERE id = ?");
                        $stmt->execute([$new_remaining_monthly, $budget['id']]);
                    } else {
                        throw new Exception('No budget allocated for fuel transaction.');
                    }
                }
            } else {
                throw new Exception('Fuel budget code "Sansii kee Sukutih 6217" not found.');
            }

            if (isset($_POST['id']) && $_POST['action'] == 'update') {
                $stmt = $pdo->prepare("UPDATE fuel_transactions SET owner_id = ?, driver_name = ?, plate_number = ?, current_gauge = ?, journey_distance = ?, fuel_price = ?, refuelable_amount = ?, total_amount = ?, new_gauge = ?, gauge_gap = ? WHERE id = ?");
                $stmt->execute([$owner_id, $driver_name, $plate_number, $current_gauge, $journey_distance, $fuel_price, $refuelable_amount, $total_amount, $new_gauge, $gauge_gap, $_POST['id']]);
                $message = 'Fuel transaction updated';
            } else {
                $stmt = $pdo->prepare("INSERT INTO fuel_transactions (owner_id, driver_name, plate_number, previous_gauge, current_gauge, journey_distance, fuel_price, refuelable_amount, total_amount, new_gauge, gauge_gap) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$owner_id, $driver_name, $plate_number, $last_new_gauge, $current_gauge, $journey_distance, $fuel_price, $refuelable_amount, $total_amount, $new_gauge, $gauge_gap]);
...
<body>
            ...
            <div class="card-body">
                ...
                <form method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="plate_number" class="form-control" value="<?php echo $fuel ? $fuel['plate_number'] : ''; ?>" required>
                    </div>
                    <div class="mb-3">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php foreach ($fuel_trans as $f): ?>
                            <tr>
                                <td><?php echo $f['id']; ?></td>
                                <td><?php echo $f['owner_code']; ?></td>
                                <td><?php echo $f['driver_name']; ?></td>
                                <td><?php echo $f['plate_number']; ?></td>
    ...
    <script>
        function calculateFuel() {
            ...
            const refuelable = journey / 10; // Assuming 10 km per liter as an example
