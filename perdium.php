...
// Ethiopian month conversion helper
...
function reverseAllocations(PDO $pdo, int $perdium_id): int {
    ...
    foreach ($rows as $al) {
        ...
        } elseif ($al['budget_type'] === 'program' && $al['gov_budget_id']) {
            // Programs now use gov_budget_id pointing to budgets table
            $r = $pdo->prepare("SELECT id FROM budgets WHERE id = ? FOR UPDATE");
            $r->execute([(int)$al['gov_budget_id']]);
...
function reverseLegacy(PDO $pdo, array $tx): void {
   ...
   if ($tx['budget_type'] === 'program') {
    ...
    // ... rest of code
        ...
        // Governmental - only monthly, no yearly fallback
        $month = $tx['et_month'];
        if (!empty($month)) {
            ...
            $r = $pdo->prepare("
                ...
                FOR UPDATE
            ");
            $r->execute([$owner_id, $code_id, $year, $ec_month]);
            $b = $r->fetch(PDO::FETCH_ASSOC);
            if ($b) {
                $pdo->prepare("UPDATE budgets SET remaining_monthly = remaining_monthly + ? WHERE id = ?")
                ->execute([$amount, (int)$b['id']]);
...
function allocateProgram(PDO $pdo, int $owner_id, int $year, float $amount): array {
    ...
    if (!$row) {
        throw new Exception('No program budget allocated or registered for this owner/year.');
    }

    $newRem = (float)$row['remaining_yearly'] - $amount;
    if ($newRem < 0) {
        throw new Exception('Insufficient program yearly budget.');
    }
...
function allocateGovernment(PDO $pdo, int $owner_id, int $year, string $et_month, float $amount, int $code_id = 6): array {
    // Convert Ethiopian month name to numeric if needed
    $ec_month = ecMonthNoFromString($et_month);
    if (!$ec_month) {
        throw new Exception('Invalid Ethiopian month provided.');
    }

    // Only use monthly budget - no yearly fallback
    $s = $pdo->prepare("
        SELECT id, remaining_monthly
        FROM budgets
        WHERE budget_type='governmental'
          AND owner_id = ?
          AND code_id = ?
        ...
        FOR UPDATE
    ");
    $s->execute([$owner_id, $code_id, $year, $ec_month]);
    $b = $s->fetch(PDO::FETCH_ASSOC);

    if (!$b) {
        throw new Exception('No monthly perdium budget allocated for selected owner/month.');
    }
...
<head>
    ...
    <style>
        ...
        /* Center dialogs in main content area */
        .main-content-centered {
            ...
            /* Half of sidebar width to adjust for sidebar */
        }

        /* When sidebar is collapsed */
        .sidebar.collapsed ~ .main-content .main-content-centered {
            margin-left: 40px !important;
            /* Half of collapsed sidebar width */
        }
        ...
        /* SweetAlert2 popup adjustments */
        .swal2-popup.main-content-centered {
        ...
        /* InsufficientBadget CSS*/

        /* Budget shortage styling */
        .budget-shortage {
...
<script>
    ...
    function validateBeforeSubmit() {
        ...
        const remainingMonthly = parseFloat($('#rem_monthly').text().replace(/,/g, '')) || 0;

        if (budgetType === 'governmental' && ownerId && etMonth && totalAmount > 0) {
            if (totalAmount > remainingMonthly) {
                showInsufficientBudgetDialog(
                    'Insufficient monthly budget for this transaction.',
                    remainingMonthly,
                    totalAmount
                );
                return false;
    ...
    // show InsufficientBadget //

    function showInsufficientBudgetDialog(message, remainingMonthly, requiredAmount) {
        const shortage = (requiredAmount - remainingMonthly).toFixed(2);

        // Calculate the offset to center in the main content area
        ...
        // Calculate the horizontal offset to center in main content
        const horizontalOffset = (sidebarWidth / 2) - (mainContentWidth / 4);

        Swal.fire({
            title: '<div class="flex items-center justify-center mb-4">' +
            '<img src="images/bureau-logo.png" class="h-16 mr-3 bureau-logo" alt="Bureau Logo">' +
            '<span class="text-2xl font-bold text-gray-800">Insufficient Budget</span>' +
            '</div>',
            html: `
            <div class="text-center py-4">
            ...
            <h3 class="text-xl font-bold text-gray-800 mb-3">Budget Shortage Detected</h3>

            <div class="bg-gradient-to-r from-red-50 to-orange-50 border border-red-200 rounded-xl p-4 mb-4">
            <div class="grid grid-cols-2 gap-4 text-left">
            <div class="text-center">
            <div class="text-sm text-gray-600 font-medium">Required Amount</div>
            <div class="text-lg font-bold text-red-600">${birr(requiredAmount)}</div>
            </div>
            <div class="text-center">
            <div class="text-sm text-gray-600 font-medium">Available Budget</div>
            <div class="text-lg font-bold text-green-600">${birr(remainingMonthly)}</div>
            </div>
            </div>
            <div class="mt-3 p-3 bg-white rounded-lg border border-red-300">
            <div class="flex items-center justify-center">
            <i class="fas fa-minus-circle text-red-500 mr-2"></i>
            <span class="text-red-700 font-semibold">Shortage: ${birr(shortage)}</span>
            </div>
            </div>
            </div>
            ...
            The Remaining Monthly Perdium Budget Allocated for this Directorate/Program is not sufficient to cover this transaction.
            </p>

            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
            <h4 class="font-semibold text-blue-800 mb-2 flex items-center">
            <i class="fas fa-lightbulb mr-2"></i>Recommended Actions:
            </h4>
            <ul class="text-left text-blue-700 text-sm space-y-1">
            <li><i class="fas fa-check-circle text-green-500 mr-2"></i>Allocate additional budget to this directorate</li>
            <li><i class="fas fa-check-circle text-green-500 mr-2"></i>Reduce the perdium amount or duration</li>
            <li><i class="fas fa-check-circle text-green-500 mr-2"></i>Use a different budget source if available</li>
            </ul>
            </div>

            <div class="text-xs text-gray-500 mt-4">
            <i class="fas fa-clock mr-1"></i>This transaction requires additional budget allocation to proceed.
            </div>
            </div>
            `,
            showCancelButton: true,
            confirmButtonText: '<i class="fas fa-plus-circle mr-2"></i>Add Budget',
            cancelButtonText: '<i class="fas fa-check mr-2"></i>Understand',
            confirmButtonColor: '#3b82f6',
            cancelButtonColor: '#6b7280',
            background: '#ffffff',
            width: '600px',
            customClass: {
                popup: 'rounded-2xl shadow-2xl border border-gray-200 main-content-centered',
                confirmButton: 'px-6 py-3 rounded-lg font-semibold shadow-lg hover:shadow-xl transition-all duration-200',
                cancelButton: 'px-6 py-3 rounded-lg font-semibold'
            },
            showClass: {
            ...
            didOpen: () => {
                // Center the dialog in the main content area
                setTimeout(() => {
                    const popup = Swal.getPopup();
                    if (popup) {
                        ...
                        // Calculate center position within main content
                        const mainContentCenter = mainContentRect.left + (mainContentRect.width / 2);
                        const targetLeft = mainContentCenter - (popupRect.width / 2);
            ...
            if (result.isConfirmed) {
                // Redirect to budget adding page
                window.location.href = 'Budget_adding.php';
            }
        });
    }
</script>
